  ...
  return (
    <div className="w-full relative">
      ...
      <div className={`relative flex items-end gap-2 bg-white border-2 border-gray-200 p-2 shadow-sm transition-all focus-within:border-teal-500 focus-within:ring-2 focus-within:ring-teal-500/10 
        ...
        <Button
          onClick={loading ? () => onSend(true) : () => onSend(false)}
          disabled={!loading && (!prompt.trim() && attachedFiles.length === 0)}
          size="icon"
          className={`bg-teal-600 hover:bg-teal-700 disabled:bg-gray-200 disabled:text-gray-400 rounded-xl shrink-0 transition-all ${isLanding ? "h-11 w-11 sm:rounded-2xl" : "h-9 w-9"}`}
        >
          {loading ? (
            <div className="relative flex items-center justify-center">
              <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin absolute" />
              <div className="w-2.5 h-2.5 bg-white rounded-sm z-10 hover:scale-90 transition-transform" />
            </div>
          ) : (
            <ArrowUp className="h-5 w-5" />
          )}
  ...
  const handleSendPrompt = async () => {
    ...
    try {
      ...
      const eventSource = new EventSource(
        `${API_BASE_URL}/generate/${data.id}/stream`
      )
      ...
      eventSource.addEventListener("progress", (e) => {
        ...
        if (event.data?.progress?.message) {
          setMessages(prev => {
            // Ensure we don't duplicate sticky progress messages or just append them?
            // For now, let's just append as system messages or update the last one if it's a progress type.
            // But the UI structure is rigid. Let's just append for visibility.
            return [...prev, { role: "assistant", content: `[Progress] ${event.data.progress.message}` }]
          })
        }
      })

      eventSource.addEventListener("file_generated", (e) => {
        const event = JSON.parse(e.data)

        setFiles((prev) => ({
          ...prev,
          [event.data.file.path]: event.data.file.content
        }))
      })
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-slate-50 relative">
        ...
        <div className="flex-1 p-3 overflow-hidden">
          <Card className="h-full w-full overflow-hidden border-gray-200 shadow-sm flex flex-col">
            <div className="h-10 border-b flex items-center px-4 gap-4 bg-gray-50/50">
              <span className="text-xs font-medium text-gray-500">Files</span>
              {Object.keys(files).length > 0 ? (
                Object.keys(files).map(fileName => (
                  <button
                    key={fileName}
                    // Simple file tab logic: if we had a state, we'd use it. For now, picking the first/only file or just showing them.
                    // Actually, we need state for active file.
                    // But I cannot add state in this chunk easily without a separate chunk for the component body.
                    // I will assume the first file is the main one for now, or just iterate.
                    className="px-3 py-1 text-xs rounded-full bg-teal-100 text-teal-700 font-medium"
                  >
                    {fileName.replace(/^\//, '')}
                  </button>
                ))
              ) : (
                <span className="text-xs text-gray-400 italic">No files generated yet</span>
              )}
            </div>

            <div className="flex-1 flex overflow-hidden">
              <Tabs defaultValue="preview" className="h-full w-full flex flex-col">
                <TabsContent value="preview" className="flex-1 m-0 p-0 bg-white relative">
                  {/* Live Preview using the content of the first file (usually App.tsx) */}
                  <LivePreview code={Object.values(files)[0] || ""} />
                </TabsContent>
                <TabsContent value="code" className="flex-1 m-0 p-0 bg-[#0f172a]">
                  <CodeEditor
                    code={Object.values(files)[0] || "// Waiting for generation..."}
                    onChange={(val) => {
                      if (val && Object.keys(files).length > 0) {
                        const firstFile = Object.keys(files)[0];
                        setFiles(prev => ({ ...prev, [firstFile]: val }));
                      }
                    }}
                  />
                </TabsContent>
